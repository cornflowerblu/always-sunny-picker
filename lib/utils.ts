...
export async function episodeFilter(req: Request): Promise<any> {
  const shows = await getShows({}, client.adminRequestHeaders).catch(err => err);
  const singleShow = await getSingleShow({ id: req.params.showId }, client.adminRequestHeaders).catch(err => err);
  const seasons = await getSeasons({ showId: req.params.showId }, client.adminRequestHeaders).catch(err => err);
  const singleSeason = await getSingleSeason({ id: req.params.seasonId }, client.adminRequestHeaders).catch(err => err);
  const episodes = await getEpisodesBySeason({ seasonId: req.params.seasonId }, client.adminRequestHeaders).catch(err => err);
  const episode = await getSingleEpisode({ id: req.params.episodeId }, client.adminRequestHeaders).catch(err => err);
